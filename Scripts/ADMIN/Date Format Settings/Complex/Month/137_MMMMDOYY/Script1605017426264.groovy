...
WebUI.verifyElementAttributeValue(findTestObject('ADMIN/Date Format Settings/button_DD'), 'value', 'MMMM', 0)
...
def formattedDate = date.format('MMMM/d/yy')
...
switch (true) {
    case day.charAt(0) == 1:
        converted_date = (((((month + '/') + day) + 'st') + '/') + year)

        println(converted_date)

        break
    case day.charAt(0) == 2:
        converted_date = (((((month + '/') + day) + 'nd') + '/') + year)

        println(converted_date)

        break
    case day.charAt(0) == 3:
        converted_date = (((((month + '/') + day) + 'rd') + '/') + year)

        println(converted_date)

        break
    default:
        converted_date = (((((month + '/') + day) + 'th') + '/') + year)

        println(converted_date)}

PreviewDatetmp.text = converted_date
DateFormattmp.text = 'MMMM/d/yy'
