...
WebUI.setText(findTestObject('FIND/BUYER/Checkout/searchbox_homepage'), 'Item_03')
...
WebUI.callTestCase(findTestCase('FIND/Buyer/variantScenario'), [:], FailureHandling.CONTINUE_ON_FAILURE)
...
def dfsub = df.format(new BigDecimal(subtotal))

subtotalTmp.text = dfsub

WebUI.click(findTestObject('FIND/BUYER/Checkout/btn_Add to Cart'))
...
WebUI.selectOptionByLabel(findTestObject('FIND/BUYER/Checkout/dd_paymentMethod'), 'Offline Payments 1', false)
...
WebUI.waitForElementVisible(findTestObject('FIND/BUYER/Checkout/textvalue_invoice'), 0)
